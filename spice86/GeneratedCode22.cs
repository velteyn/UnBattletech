...
public partial class GeneratedOverrides : CSharpOverrideHelper {
  ...
  public virtual Action ghidra_guess_1000_D45F_1D45F(int loadOffset) {
    ...
    CheckExternalEvents(cs8, 0xD462);
    // MOV AX,[0x52f3] (1000_D45F / 0x1D45F)
    AX = UInt16[DS, 0x52F3];
    CheckExternalEvents(cs8, 0xD464);
    // NOT AX (1000_D462 / 0x1D462)
    AX = (ushort)(~AX);
    CheckExternalEvents(cs8, 0xD466);
    // AND AX,CX (1000_D464 / 0x1D464)
    AX &= CX;
    CheckExternalEvents(cs8, 0xD468);
    // XOR CX,CX (1000_D466 / 0x1D466)
    CX = 0;
    CheckExternalEvents(cs8, 0xD46A);
    // TEST AL,0x80 (1000_D468 / 0x1D468)
    Alu8.And(AL, 0x80);
    CheckExternalEvents(cs8, 0xD46C);
    // JNZ 0x1000:d46f (1000_D46A / 0x1D46A)
    if(!ZeroFlag) {
      // JNZ target is RET, inlining.
      CheckExternalEvents(cs8, 0xD470);
      // RET  (1000_D46F / 0x1D46F)
      return NearRet();
    }
    CheckExternalEvents(cs8, 0xD46F);
  ...
  public virtual Action unknown_19EF_3580_1D470(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3597);
    // XOR AX,AX (19EF_3595 / 0x1D485)
    // AX ^= AX;
    AX = Alu16.Xor(AX, AX);
    CheckExternalEvents(cs14, 0x359A);
    // MOV CX,word ptr [BP + 0xc] (19EF_3597 / 0x1D487)
    CX = UInt16[SS, (ushort)(BP + 0xC)];
    CheckExternalEvents(cs14, 0x359C);
    ...
    CheckExternalEvents(cs14, 0x35A1);
    // TEST byte ptr [BX + 0x5300],0x2 (19EF_359C / 0x1D48C)
    Alu8.And(UInt8[DS, (ushort)(BX + 0x5300)], 0x2);
    CheckExternalEvents(cs14, 0x35A3);
    ...
    CheckExternalEvents(cs14, 0x35BA);
    // TEST byte ptr [BX + 0x5300],0x80 (19EF_35B5 / 0x1D4A5)
    Alu8.And(UInt8[DS, (ushort)(BX + 0x5300)], 0x80);
    CheckExternalEvents(cs14, 0x35BC);
  ...
  public virtual Action ghidra_guess_1000_D55A_1D55A(int loadOffset) {
    ...
    CheckExternalEvents(cs8, 0xD575);
    // TEST byte ptr [BX + 0x5300],0x20 (1000_D570 / 0x1D570)
    Alu8.And(UInt8[DS, (ushort)(BX + 0x5300)], 0x20);
    CheckExternalEvents(cs8, 0xD577);
    // JZ 0x1000:d582 (1000_D575 / 0x1D575)
    if(ZeroFlag) {
      goto label_1000_D582_1D582;
    }
    CheckExternalEvents(cs8, 0xD57A);
    // MOV AX,0x4202 (1000_D577 / 0x1D577)
    AX = 0x4202;
    CheckExternalEvents(cs8, 0xD57C);
    // XOR CX,CX (1000_D57A / 0x1D57A)
    // CX ^= CX;
    CX = Alu16.Xor(CX, CX);
    CheckExternalEvents(cs8, 0xD57E);
    // MOV DX,CX (1000_D57C / 0x1D57C)
    DX = CX;
    CheckExternalEvents(cs8, 0xD580);
    // INT 0x21 (1000_D57E / 0x1D57E)
    Interrupt(0x21);
    CheckExternalEvents(cs8, 0xD582);
    ...
    CheckExternalEvents(cs8, 0xD587);
    // TEST byte ptr [BX + 0x5300],0x80 (1000_D582 / 0x1D582)
    Alu8.And(UInt8[DS, (ushort)(BX + 0x5300)], 0x80);
    CheckExternalEvents(cs8, 0xD589);
    // JZ 0x1000:d605 (1000_D587 / 0x1D587)
    if(ZeroFlag) {
      // JZ target is JMP, inlining.
      CheckExternalEvents(cs8, 0xD607);
    ...
    CheckExternalEvents(cs8, 0xD5AB);
    // MOV AL,0xa (1000_D5A9 / 0x1D5A9)
    AL = 0xA;
    CheckExternalEvents(cs8, 0xD5AD);
    // REPNE
    while (CX != 0) {
      CX--;
    ...
    CheckExternalEvents(cs8, 0xD60B);
    // XOR AX,AX (1000_D609 / 0x1D609)
    // AX ^= AX;
    AX = Alu16.Xor(AX, AX);
    CheckExternalEvents(cs8, 0xD60E);
  ...
  public virtual Action ghidra_guess_1000_D60E_1D60E(int loadOffset) {
    ...
    CheckExternalEvents(cs8, 0xD654);
    // STC  (1000_D653 / 0x1D653)
    CarryFlag = true;
    CheckExternalEvents(cs8, 0xD657);
    // MOV AX,0x1c00 (1000_D654 / 0x1D654)
    AX = 0x1C00;
    CheckExternalEvents(cs8, 0xD659);
  ...
  public virtual Action ghidra_guess_1000_D66A_1D66A(int loadOffset) {
    ...
    CheckExternalEvents(cs8, 0xD677);
    // PUSH DS (1000_D676 / 0x1D676)
    Stack.Push16(DS);
    CheckExternalEvents(cs8, 0xD67A);
    // LDS DX,[BP + 0x8] (1000_D677 / 0x1D677)
    DX = UInt16[SS, (ushort)(BP + 0x8)];
    DS = UInt16[SS, (ushort)(BP + 0x8 + 2)];
    CheckExternalEvents(cs8, 0xD67C);
    // MOV AH,0x40 (1000_D67A / 0x1D67A)
    AH = 0x40;
    CheckExternalEvents(cs8, 0xD67E);
    // INT 0x21 (1000_D67C / 0x1D67C)
    Interrupt(0x21);
    CheckExternalEvents(cs8, 0xD67F);
    // PUSH DS (1000_D67E / 0x1D67E)
    Stack.Push16(DS);
    CheckExternalEvents(cs8, 0xD680);
    // POP ES (1000_D67F / 0x1D67F)
    ES = Stack.Pop16();;
    CheckExternalEvents(cs8, 0xD681);
    // POP DS (1000_D680 / 0x1D680)
    DS = Stack.Pop16();;
    CheckExternalEvents(cs8, 0xD683);
    // JNC 0x1000:d687 (1000_D681 / 0x1D681)
    if(!CarryFlag) {
      goto label_1000_D687_1D687;
    }
    CheckExternalEvents(cs8, 0xD685);
    // MOV AH,0x9 (1000_D683 / 0x1D683)
    AH = 0x9;
    CheckExternalEvents(cs8, 0xD687);
    ...
    CheckExternalEvents(cs8, 0xD689);
    // OR AX,AX (1000_D687 / 0x1D687)
    // AX |= AX;
    AX = Alu16.Or(AX, AX);
    CheckExternalEvents(cs8, 0xD68B);
    ...
    CheckExternalEvents(cs8, 0xD69E);
    // STC  (1000_D69D / 0x1D69D)
    CarryFlag = true;
    CheckExternalEvents(cs8, 0xD6A1);
    // MOV AX,0x1c00 (1000_D69E / 0x1D69E)
    AX = 0x1C00;
    CheckExternalEvents(cs8, 0xD6A3);
  ...
  public virtual Action ghidra_guess_1000_D6A4_1D6A4(int loadOffset) {
    ...
    CheckExternalEvents(cs8, 0xD6A5);
    // POP CX (1000_D6A4 / 0x1D6A4)
    CX = Stack.Pop16();;
    CheckExternalEvents(cs8, 0xD6A6);
    // POP DX (1000_D6A5 / 0x1D6A5)
    DX = Stack.Pop16();;
    CheckExternalEvents(cs8, 0xD6A9);
    // MOV AX,[0x5334] (1000_D6A6 / 0x1D6A6)
    AX = UInt16[DS, 0x5334];
    CheckExternalEvents(cs8, 0xD6AB);
    // CMP AX,SP (1000_D6A9 / 0x1D6A9)
    Alu16.Sub(AX, SP);
    CheckExternalEvents(cs8, 0xD6AD);
    // JNC 0x1000:d6b4 (1000_D6AB / 0x1D6AB)
    if(!CarryFlag) {
      goto label_1000_D6B4_1D6B4;
    }
    CheckExternalEvents(cs8, 0xD6AF);
    // SUB AX,SP (1000_D6AD / 0x1D6AD)
    AX -= SP;
    CheckExternalEvents(cs8, 0xD6B1);
    // NEG AX (1000_D6AF / 0x1D6AF)
    AX = Alu16.Sub(0, AX);
    label_1000_D6B1_1D6B1:
    CheckExternalEvents(cs8, 0xD6B2);
    // PUSH DX (1000_D6B1 / 0x1D6B1)
    Stack.Push16(DX);
    CheckExternalEvents(cs8, 0xD6B3);
    // PUSH CX (1000_D6B2 / 0x1D6B2)
    Stack.Push16(CX);
    CheckExternalEvents(cs8, 0xD6B4);
    // RETF  (1000_D6B3 / 0x1D6B3)
    return FarRet();
    label_1000_D6B4_1D6B4:
    CheckExternalEvents(cs8, 0xD6B6);
    // XOR AX,AX (1000_D6B4 / 0x1D6B4)
    // AX ^= AX;
    AX = Alu16.Xor(AX, AX);
    CheckExternalEvents(cs8, 0xD6B8);
  ...
  public virtual Action unknown_19EF_3835_1D725(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3836);
    // PUSH BP (19EF_3835 / 0x1D725)
    Stack.Push16(BP);
    CheckExternalEvents(cs14, 0x3838);
    // MOV BP,SP (19EF_3836 / 0x1D726)
    BP = SP;
    CheckExternalEvents(cs14, 0x383B);
    // SUB SP,0x2 (19EF_3838 / 0x1D728)
    // SP -= 0x2;
    SP = Alu16.Sub(SP, 0x2);
    CheckExternalEvents(cs14, 0x383C);
    // PUSH SI (19EF_383B / 0x1D72B)
    Stack.Push16(SI);
    CheckExternalEvents(cs14, 0x383D);
    // PUSH DI (19EF_383C / 0x1D72C)
    Stack.Push16(DI);
    CheckExternalEvents(cs14, 0x3840);
    // MOV AX,word ptr [BP + 0x6] (19EF_383D / 0x1D72D)
    AX = UInt16[SS, (ushort)(BP + 0x6)];
    CheckExternalEvents(cs14, 0x3843);
    // CMP AX,0xfff1 (19EF_3840 / 0x1D730)
    Alu16.Sub(AX, 0xFFF1);
    CheckExternalEvents(cs14, 0x3845);
    // JNC 0x1000:d753 (19EF_3843 / 0x1D733)
    if(!CarryFlag) {
      goto label_19EF_3863_1D753;
    }
    CheckExternalEvents(cs14, 0x384A);
    // CMP word ptr [0x535a],0x0 (19EF_3845 / 0x1D735)
    Alu16.Sub(UInt16[DS, 0x535A], 0x0);
    CheckExternalEvents(cs14, 0x384C);
    // JNZ 0x1000:d744 (19EF_384A / 0x1D73A)
    if(!ZeroFlag) {
      goto label_19EF_3854_1D744;
    }
    CheckExternalEvents(cs14, 0x384F);
    // CALL 0x1000:d764 (19EF_384C / 0x1D73C)
    NearCall(cs14, 0x384F, unknown_19EF_3874_1D764);
    CheckExternalEvents(cs14, 0x3851);
    // JZ 0x1000:d753 (19EF_384F / 0x1D73F)
    if(ZeroFlag) {
      goto label_19EF_3863_1D753;
    }
    CheckExternalEvents(cs14, 0x3854);
    // MOV [0x535a],AX (19EF_3851 / 0x1D741)
    UInt16[DS, 0x535A] = AX;
    label_19EF_3854_1D744:
    CheckExternalEvents(cs14, 0x3857);
    // CALL 0x1000:d7d2 (19EF_3854 / 0x1D744)
    NearCall(cs14, 0x3857, unknown_19EF_38E2_1D7D2);
    CheckExternalEvents(cs14, 0x3859);
    // JNZ 0x1000:d75e (19EF_3857 / 0x1D747)
    if(!ZeroFlag) {
      goto label_19EF_386E_1D75E;
    }
    CheckExternalEvents(cs14, 0x385C);
    // CALL 0x1000:d764 (19EF_3859 / 0x1D749)
    NearCall(cs14, 0x385C, unknown_19EF_3874_1D764);
    CheckExternalEvents(cs14, 0x385E);
    // JZ 0x1000:d753 (19EF_385C / 0x1D74C)
    if(ZeroFlag) {
      goto label_19EF_3863_1D753;
    }
    CheckExternalEvents(cs14, 0x3861);
    // CALL 0x1000:d7d2 (19EF_385E / 0x1D74E)
    NearCall(cs14, 0x3861, unknown_19EF_38E2_1D7D2);
    CheckExternalEvents(cs14, 0x3863);
    // JNZ 0x1000:d75e (19EF_3861 / 0x1D751)
    if(!ZeroFlag) {
      goto label_19EF_386E_1D75E;
    }
    label_19EF_3863_1D753:
    CheckExternalEvents(cs14, 0x3866);
    // PUSH word ptr [BP + 0x6] (19EF_3863 / 0x1D753)
    Stack.Push16(UInt16[SS, (ushort)(BP + 0x6)]);
    CheckExternalEvents(cs14, 0x386B);
    // CALLF 0x1000:d6ca (19EF_3866 / 0x1D756)
    FarCall(cs14, 0x386B, ghidra_guess_1000_D6CA_1D6CA);
    CheckExternalEvents(cs14, 0x386E);
    // ADD SP,0x2 (19EF_386B / 0x1D75B)
    // SP += 0x2;
    SP = Alu16.Add(SP, 0x2);
    label_19EF_386E_1D75E:
    CheckExternalEvents(cs14, 0x386F);
    // POP DI (19EF_386E / 0x1D75E)
    DI = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3870);
    // POP SI (19EF_386F / 0x1D75F)
    SI = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3872);
    // MOV SP,BP (19EF_3870 / 0x1D760)
    SP = BP;
    CheckExternalEvents(cs14, 0x3873);
    // POP BP (19EF_3872 / 0x1D762)
    BP = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3874);
  ...
  public virtual Action unknown_19EF_38E2_1D7D2(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x38E4);
    // MOV AX,DS (19EF_38E2 / 0x1D7D2)
    AX = DS;
    CheckExternalEvents(cs14, 0x38E6);
    // MOV ES,AX (19EF_38E4 / 0x1D7D4)
    ES = AX;
    CheckExternalEvents(cs14, 0x38E9);
    // MOV CX,word ptr [BP + 0x6] (19EF_38E6 / 0x1D7D6)
    CX = UInt16[SS, (ushort)(BP + 0x6)];
    CheckExternalEvents(cs14, 0x38EB);
    // XOR BX,BX (19EF_38E9 / 0x1D7D9)
    // BX ^= BX;
    BX = Alu16.Xor(BX, BX);
    CheckExternalEvents(cs14, 0x38EF);
    // MOV DS,word ptr [0x535e] (19EF_38EB / 0x1D7DB)
    DS = UInt16[DS, 0x535E];
    CheckExternalEvents(cs14, 0x38F2);
    // CALL 0x1000:d7ed (19EF_38EF / 0x1D7DF)
    NearCall(cs14, 0x38F2, unknown_19EF_38FD_1D7ED);
    CheckExternalEvents(cs14, 0x38F4);
    // OR DX,DX (19EF_38F2 / 0x1D7E2)
    // DX |= DX;
    DX = Alu16.Or(DX, DX);
    CheckExternalEvents(cs14, 0x38F6);
    // MOV CX,ES (19EF_38F4 / 0x1D7E4)
    CX = ES;
    CheckExternalEvents(cs14, 0x38F8);
    // MOV DS,CX (19EF_38F6 / 0x1D7E6)
    DS = CX;
    CheckExternalEvents(cs14, 0x38F9);
  ...
  public virtual Action unknown_19EF_38FD_1D7ED(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3903);
    // AND CL,0xfe (19EF_3900 / 0x1D7F0)
    CL &= 0xFE;
    CheckExternalEvents(cs14, 0x3906);
    // CMP CX,-0x12 (19EF_3903 / 0x1D7F3)
    Alu16.Sub(CX, 0xFFEE);
    CheckExternalEvents(cs14, 0x3908);
  ...
  public virtual Action unknown_19EF_3A1A_1D90A(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3A1B);
    // PUSH DX (19EF_3A1A / 0x1D90A)
    Stack.Push16(DX);
    CheckExternalEvents(cs14, 0x3A1C);
    // PUSH CX (19EF_3A1B / 0x1D90B)
    Stack.Push16(CX);
    CheckExternalEvents(cs14, 0x3A1F);
    // CALL 0x1000:d92c (19EF_3A1C / 0x1D90C)
    NearCall(cs14, 0x3A1F, unknown_19EF_3A3C_1D92C);
    CheckExternalEvents(cs14, 0x3A21);
    // JZ 0x1000:d929 (19EF_3A1F / 0x1D90F)
    if(ZeroFlag) {
      goto label_19EF_3A39_1D929;
    }
    CheckExternalEvents(cs14, 0x3A22);
    // PUSH DI (19EF_3A21 / 0x1D911)
    Stack.Push16(DI);
    CheckExternalEvents(cs14, 0x3A24);
    // MOV DI,SI (19EF_3A22 / 0x1D912)
    DI = SI;
    CheckExternalEvents(cs14, 0x3A26);
    // MOV SI,AX (19EF_3A24 / 0x1D914)
    SI = AX;
    CheckExternalEvents(cs14, 0x3A28);
    // ADD SI,DX (19EF_3A26 / 0x1D916)
    // SI += DX;
    SI = Alu16.Add(SI, DX);
    CheckExternalEvents(cs14, 0x3A2D);
    // MOV word ptr [SI + -0x2],0xfffe (19EF_3A28 / 0x1D918)
    UInt16[DS, (ushort)(SI - 0x2)] = 0xFFFE;
    CheckExternalEvents(cs14, 0x3A30);
    // MOV word ptr [BX + 0x6],SI (19EF_3A2D / 0x1D91D)
    UInt16[DS, (ushort)(BX + 0x6)] = SI;
    CheckExternalEvents(cs14, 0x3A32);
    // MOV DX,SI (19EF_3A30 / 0x1D920)
    DX = SI;
    CheckExternalEvents(cs14, 0x3A34);
    // SUB DX,DI (19EF_3A32 / 0x1D922)
    DX -= DI;
    CheckExternalEvents(cs14, 0x3A35);
    // DEC DX (19EF_3A34 / 0x1D924)
    DX = Alu16.Dec(DX);
    CheckExternalEvents(cs14, 0x3A38);
    // MOV word ptr [DI + -0x2],DX (19EF_3A35 / 0x1D925)
    UInt16[DS, (ushort)(DI - 0x2)] = DX;
    CheckExternalEvents(cs14, 0x3A39);
    // POP AX (19EF_3A38 / 0x1D928)
    AX = Stack.Pop16();;
    label_19EF_3A39_1D929:
    CheckExternalEvents(cs14, 0x3A3A);
    // POP CX (19EF_3A39 / 0x1D929)
    CX = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3A3B);
    // POP DX (19EF_3A3A / 0x1D92A)
    DX = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3A3C);
  ...
  public virtual Action unknown_19EF_3A3C_1D92C(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3A3D);
    // PUSH BX (19EF_3A3C / 0x1D92C)
    Stack.Push16(BX);
    CheckExternalEvents(cs14, 0x3A3E);
    // PUSH AX (19EF_3A3D / 0x1D92D)
    Stack.Push16(AX);
    CheckExternalEvents(cs14, 0x3A40);
    // XOR DX,DX (19EF_3A3E / 0x1D92E)
    // DX ^= DX;
    DX = Alu16.Xor(DX, DX);
    CheckExternalEvents(cs14, 0x3A41);
    // PUSH DS (19EF_3A40 / 0x1D930)
    Stack.Push16(DS);
    CheckExternalEvents(cs14, 0x3A42);
    // PUSH DX (19EF_3A41 / 0x1D931)
    Stack.Push16(DX);
    CheckExternalEvents(cs14, 0x3A43);
    // PUSH DX (19EF_3A42 / 0x1D932)
    Stack.Push16(DX);
    CheckExternalEvents(cs14, 0x3A44);
    // PUSH AX (19EF_3A43 / 0x1D933)
    Stack.Push16(AX);
    CheckExternalEvents(cs14, 0x3A47);
    // MOV AX,0x1 (19EF_3A44 / 0x1D934)
    AX = 0x1;
    CheckExternalEvents(cs14, 0x3A48);
    // PUSH AX (19EF_3A47 / 0x1D937)
    Stack.Push16(AX);
    CheckExternalEvents(cs14, 0x3A49);
    // PUSH ES (19EF_3A48 / 0x1D938)
    Stack.Push16(ES);
    CheckExternalEvents(cs14, 0x3A4A);
    // POP DS (19EF_3A49 / 0x1D939)
    DS = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3A4F);
    // CALLF 0x1000:d94e (19EF_3A4A / 0x1D93A)
    FarCall(cs14, 0x3A4F, unknown_19EF_3A5E_1D94E);
    CheckExternalEvents(cs14, 0x3A52);
    // ADD SP,0x8 (19EF_3A4F / 0x1D93F)
    SP += 0x8;
    CheckExternalEvents(cs14, 0x3A55);
    // CMP DX,-0x1 (19EF_3A52 / 0x1D942)
    Alu16.Sub(DX, 0xFFFF);
    CheckExternalEvents(cs14, 0x3A56);
    // POP DS (19EF_3A55 / 0x1D945)
    DS = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3A57);
    // POP DX (19EF_3A56 / 0x1D946)
    DX = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3A58);
    // POP BX (19EF_3A57 / 0x1D947)
    BX = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3A5A);
    // JZ 0x1000:d94c (19EF_3A58 / 0x1D948)
    if(ZeroFlag) {
      // JZ target is RET, inlining.
      CheckExternalEvents(cs14, 0x3A5D);
      // RET  (19EF_3A5C / 0x1D94C)
      return NearRet();
    }
    CheckExternalEvents(cs14, 0x3A5C);
  ...
  public virtual Action unknown_19EF_3B22_1DA12(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3B23);
    // PUSH BP (19EF_3B22 / 0x1DA12)
    Stack.Push16(BP);
    CheckExternalEvents(cs14, 0x3B25);
    // MOV BP,SP (19EF_3B23 / 0x1DA13)
    BP = SP;
    CheckExternalEvents(cs14, 0x3B27);
    // MOV DX,DI (19EF_3B25 / 0x1DA15)
    DX = DI;
    CheckExternalEvents(cs14, 0x3B29);
    // MOV BX,SI (19EF_3B27 / 0x1DA17)
    BX = SI;
    CheckExternalEvents(cs14, 0x3B2A);
    // PUSH DS (19EF_3B29 / 0x1DA19)
    Stack.Push16(DS);
    CheckExternalEvents(cs14, 0x3B2D);
    // LES DI,[BP + 0x6] (19EF_3B2A / 0x1DA1A)
    DI = UInt16[SS, (ushort)(BP + 0x6)];
    ES = UInt16[SS, (ushort)(BP + 0x6 + 2)];
    CheckExternalEvents(cs14, 0x3B2F);
    // XOR AX,AX (19EF_3B2D / 0x1DA1D)
    // AX ^= AX;
    AX = Alu16.Xor(AX, AX);
    CheckExternalEvents(cs14, 0x3B32);
    // MOV CX,0xffff (19EF_3B2F / 0x1DA1F)
    CX = 0xFFFF;
    CheckExternalEvents(cs14, 0x3B34);
    // REPNE
    while (CX != 0) {
      CX--;
    ...
    CheckExternalEvents(cs14, 0x3B5D);
    // ADC CX,CX (19EF_3B5B / 0x1DA4B)
    CX = Alu16.Adc(CX, CX);
    CheckExternalEvents(cs14, 0x3B5F);
    // REP
    while (CX != 0) {
      CX--;
    ...
    CheckExternalEvents(cs14, 0x3B61);
    // MOV SI,BX (19EF_3B5F / 0x1DA4F)
    SI = BX;
    CheckExternalEvents(cs14, 0x3B63);
    // MOV DI,DX (19EF_3B61 / 0x1DA51)
    DI = DX;
    CheckExternalEvents(cs14, 0x3B64);
    // POP DS (19EF_3B63 / 0x1DA53)
    DS = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3B66);
    // MOV DX,ES (19EF_3B64 / 0x1DA54)
    DX = ES;
    CheckExternalEvents(cs14, 0x3B67);
    // POP BP (19EF_3B66 / 0x1DA56)
    BP = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3B68);
  ...
  public virtual Action unknown_19EF_3B68_1DA58(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3B69);
    // PUSH BP (19EF_3B68 / 0x1DA58)
    Stack.Push16(BP);
    CheckExternalEvents(cs14, 0x3B6B);
    // MOV BP,SP (19EF_3B69 / 0x1DA59)
    BP = SP;
    CheckExternalEvents(cs14, 0x3B6D);
    // MOV DX,DI (19EF_3B6B / 0x1DA5B)
    DX = DI;
    CheckExternalEvents(cs14, 0x3B6F);
    // MOV BX,SI (19EF_3B6D / 0x1DA5D)
    BX = SI;
    CheckExternalEvents(cs14, 0x3B70);
    // PUSH DS (19EF_3B6F / 0x1DA5F)
    Stack.Push16(DS);
    CheckExternalEvents(cs14, 0x3B73);
    // LDS SI,[BP + 0xa] (19EF_3B70 / 0x1DA60)
    SI = UInt16[SS, (ushort)(BP + 0xA)];
    DS = UInt16[SS, (ushort)(BP + 0xA + 2)];
    CheckExternalEvents(cs14, 0x3B75);
    // MOV DI,SI (19EF_3B73 / 0x1DA63)
    DI = SI;
    CheckExternalEvents(cs14, 0x3B77);
    // MOV AX,DS (19EF_3B75 / 0x1DA65)
    AX = DS;
    CheckExternalEvents(cs14, 0x3B79);
    // MOV ES,AX (19EF_3B77 / 0x1DA67)
    ES = AX;
    CheckExternalEvents(cs14, 0x3B7B);
    // XOR AX,AX (19EF_3B79 / 0x1DA69)
    // AX ^= AX;
    AX = Alu16.Xor(AX, AX);
    CheckExternalEvents(cs14, 0x3B7E);
    // MOV CX,0xffff (19EF_3B7B / 0x1DA6B)
    CX = 0xFFFF;
    CheckExternalEvents(cs14, 0x3B80);
    // REPNE
    while (CX != 0) {
      CX--;
    ...
    CheckExternalEvents(cs14, 0x3B82);
    // NOT CX (19EF_3B80 / 0x1DA70)
    CX = (ushort)(~CX);
    CheckExternalEvents(cs14, 0x3B85);
    // LES DI,[BP + 0x6] (19EF_3B82 / 0x1DA72)
    DI = UInt16[SS, (ushort)(BP + 0x6)];
    ES = UInt16[SS, (ushort)(BP + 0x6 + 2)];
    CheckExternalEvents(cs14, 0x3B87);
    // MOV AX,DI (19EF_3B85 / 0x1DA75)
    AX = DI;
    CheckExternalEvents(cs14, 0x3B89);
    // TEST AL,0x1 (19EF_3B87 / 0x1DA77)
    Alu8.And(AL, 0x1);
    CheckExternalEvents(cs14, 0x3B8B);
    // JZ 0x1000:da7d (19EF_3B89 / 0x1DA79)
    if(ZeroFlag) {
      goto label_19EF_3B8D_1DA7D;
    }
    CheckExternalEvents(cs14, 0x3B8C);
    // MOVSB ES:DI,SI (19EF_3B8B / 0x1DA7B)
    UInt8[ES, (ushort)(DI)] = UInt8[DS, (ushort)(SI)];
    SI = (ushort)(SI + Direction8);
    DI = (ushort)(DI + Direction8);
    CheckExternalEvents(cs14, 0x3B8D);
    // DEC CX (19EF_3B8C / 0x1DA7C)
    CX--;
    label_19EF_3B8D_1DA7D:
    CheckExternalEvents(cs14, 0x3B8F);
    // SHR CX,0x1 (19EF_3B8D / 0x1DA7D)
    // CX >>= 0x1;
    CX = Alu16.Shr(CX, 0x1);
    CheckExternalEvents(cs14, 0x3B91);
    // REP
    while (CX != 0) {
      CX--;
    ...
    CheckExternalEvents(cs14, 0x3B93);
    // ADC CX,CX (19EF_3B91 / 0x1DA81)
    CX = Alu16.Adc(CX, CX);
    CheckExternalEvents(cs14, 0x3B95);
    // REP
    while (CX != 0) {
      CX--;
    ...
    CheckExternalEvents(cs14, 0x3B97);
    // MOV SI,BX (19EF_3B95 / 0x1DA85)
    SI = BX;
    CheckExternalEvents(cs14, 0x3B99);
    // MOV DI,DX (19EF_3B97 / 0x1DA87)
    DI = DX;
    CheckExternalEvents(cs14, 0x3B9A);
    // POP DS (19EF_3B99 / 0x1DA89)
    DS = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3B9C);
    // MOV DX,ES (19EF_3B9A / 0x1DA8A)
    DX = ES;
    CheckExternalEvents(cs14, 0x3B9D);
    // POP BP (19EF_3B9C / 0x1DA8C)
    BP = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3B9E);
  ...
  public virtual Action unknown_19EF_3B9E_1DA8E(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3B9F);
    // PUSH BP (19EF_3B9E / 0x1DA8E)
    Stack.Push16(BP);
    CheckExternalEvents(cs14, 0x3BA1);
    // MOV BP,SP (19EF_3B9F / 0x1DA8F)
    BP = SP;
    CheckExternalEvents(cs14, 0x3BA3);
    // MOV DX,DI (19EF_3BA1 / 0x1DA91)
    DX = DI;
    CheckExternalEvents(cs14, 0x3BA6);
    // LES DI,[BP + 0x6] (19EF_3BA3 / 0x1DA93)
    DI = UInt16[SS, (ushort)(BP + 0x6)];
    ES = UInt16[SS, (ushort)(BP + 0x6 + 2)];
    CheckExternalEvents(cs14, 0x3BA8);
    // XOR AX,AX (19EF_3BA6 / 0x1DA96)
    // AX ^= AX;
    AX = Alu16.Xor(AX, AX);
    CheckExternalEvents(cs14, 0x3BAB);
    // MOV CX,0xffff (19EF_3BA8 / 0x1DA98)
    CX = 0xFFFF;
    CheckExternalEvents(cs14, 0x3BAD);
    // REPNE
    while (CX != 0) {
      CX--;
    ...
    CheckExternalEvents(cs14, 0x3BAF);
    // NOT CX (19EF_3BAD / 0x1DA9D)
    CX = (ushort)(~CX);
    CheckExternalEvents(cs14, 0x3BB0);
    // DEC CX (19EF_3BAF / 0x1DA9F)
    CX = Alu16.Dec(CX);
    CheckExternalEvents(cs14, 0x3BB1);
    // XCHG AX,CX (19EF_3BB0 / 0x1DAA0)
    (CX, AX) = (AX, CX);
    CheckExternalEvents(cs14, 0x3BB3);
    // MOV DI,DX (19EF_3BB1 / 0x1DAA1)
    DI = DX;
    CheckExternalEvents(cs14, 0x3BB4);
    // POP BP (19EF_3BB3 / 0x1DAA3)
    BP = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3BB5);
  ...
  public virtual Action unknown_19EF_3BD2_1DAC2(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3BD3);
    // PUSH BP (19EF_3BD2 / 0x1DAC2)
    Stack.Push16(BP);
    CheckExternalEvents(cs14, 0x3BD5);
    // MOV BP,SP (19EF_3BD3 / 0x1DAC3)
    BP = SP;
    CheckExternalEvents(cs14, 0x3BD6);
    // PUSH SI (19EF_3BD5 / 0x1DAC5)
    Stack.Push16(SI);
    CheckExternalEvents(cs14, 0x3BD7);
    // PUSH DI (19EF_3BD6 / 0x1DAC6)
    Stack.Push16(DI);
    CheckExternalEvents(cs14, 0x3BD9);
    // MOV BL,0x1 (19EF_3BD7 / 0x1DAC7)
    BL = 0x1;
    CheckExternalEvents(cs14, 0x3BDC);
  ...
  public virtual Action unknown_19EF_3BDC_1DACC(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3BDF);
    // MOV AX,[0x5366] (19EF_3BDC / 0x1DACC)
    AX = UInt16[DS, 0x5366];
    CheckExternalEvents(cs14, 0x3BE1);
    // OR AH,AH (19EF_3BDF / 0x1DACF)
    // AH |= AH;
    AH = Alu8.Or(AH, AH);
    CheckExternalEvents(cs14, 0x3BE3);
    // MOV AL,0xff (19EF_3BE1 / 0x1DAD1)
    AL = 0xFF;
    CheckExternalEvents(cs14, 0x3BE5);
    // JZ 0x1000:dadb (19EF_3BE3 / 0x1DAD3)
    if(ZeroFlag) {
      // JZ target is RETF, inlining.
      CheckExternalEvents(cs14, 0x3BEC);
      // RETF  (19EF_3BEB / 0x1DADB)
      return FarRet();
    }
    CheckExternalEvents(cs14, 0x3BE7);
    // MOV AH,0xb (19EF_3BE5 / 0x1DAD5)
    AH = 0xB;
    CheckExternalEvents(cs14, 0x3BE9);
    // INT 0x21 (19EF_3BE7 / 0x1DAD7)
    Interrupt(0x21);
    CheckExternalEvents(cs14, 0x3BEB);
  ...
  public virtual Action spice86_imported_label_jump_target_19EF_3C08_1DAF8(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3C3D);
    // XCHG AX,BX (19EF_3C3C / 0x1DB2C)
    (BX, AX) = (AX, BX);
    CheckExternalEvents(cs14, 0x3C3F);
    // DIV CX (19EF_3C3D / 0x1DB2D)
    uint op1Div19EF_3C3D = (uint)(DX << 16 | AX);
  ...
  public virtual Action unknown_19EF_3C82_1DB72(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3C83);
    // PUSH BP (19EF_3C82 / 0x1DB72)
    Stack.Push16(BP);
    CheckExternalEvents(cs14, 0x3C85);
    // MOV BP,SP (19EF_3C83 / 0x1DB73)
    BP = SP;
    CheckExternalEvents(cs14, 0x3C88);
    // LES DX,[BP + 0x6] (19EF_3C85 / 0x1DB75)
    DX = UInt16[SS, (ushort)(BP + 0x6)];
    ES = UInt16[SS, (ushort)(BP + 0x6 + 2)];
    CheckExternalEvents(cs14, 0x3C8C);
    // MOV word ptr [0x536a],ES (19EF_3C88 / 0x1DB78)
    UInt16[DS, 0x536A] = ES;
    CheckExternalEvents(cs14, 0x3C90);
    // MOV word ptr [0x5368],DX (19EF_3C8C / 0x1DB7C)
    UInt16[DS, 0x5368] = DX;
    CheckExternalEvents(cs14, 0x3C91);
    // PUSH DS (19EF_3C90 / 0x1DB80)
    Stack.Push16(DS);
    CheckExternalEvents(cs14, 0x3C93);
    // MOV AX,CS (19EF_3C91 / 0x1DB81)
    AX = cs14;
    CheckExternalEvents(cs14, 0x3C95);
    // MOV DS,AX (19EF_3C93 / 0x1DB83)
    DS = AX;
    CheckExternalEvents(cs14, 0x3C98);
    // MOV DX,0x3ca5 (19EF_3C95 / 0x1DB85)
    DX = 0x3CA5;
    CheckExternalEvents(cs14, 0x3C9A);
    // MOV AL,0x24 (19EF_3C98 / 0x1DB88)
    AL = 0x24;
    CheckExternalEvents(cs14, 0x3C9C);
    // MOV AH,0x25 (19EF_3C9A / 0x1DB8A)
    AH = 0x25;
    CheckExternalEvents(cs14, 0x3C9E);
    // INT 0x21 (19EF_3C9C / 0x1DB8C)
    Interrupt(0x21);
    CheckExternalEvents(cs14, 0x3C9F);
    // POP DS (19EF_3C9E / 0x1DB8E)
    DS = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3CA1);
    // XOR AX,AX (19EF_3C9F / 0x1DB8F)
    // AX ^= AX;
    AX = Alu16.Xor(AX, AX);
    CheckExternalEvents(cs14, 0x3CA3);
    // MOV SP,BP (19EF_3CA1 / 0x1DB91)
    SP = BP;
    CheckExternalEvents(cs14, 0x3CA4);
    // POP BP (19EF_3CA3 / 0x1DB93)
    BP = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3CA5);
  ...
  public virtual Action unknown_19EF_3D44_1DC34(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3D45);
    // PUSH BP (19EF_3D44 / 0x1DC34)
    Stack.Push16(BP);
    CheckExternalEvents(cs14, 0x3D47);
    // MOV BP,SP (19EF_3D45 / 0x1DC35)
    BP = SP;
    CheckExternalEvents(cs14, 0x3D4A);
    // LES BX,[BP + 0x6] (19EF_3D47 / 0x1DC37)
    BX = UInt16[SS, (ushort)(BP + 0x6)];
    ES = UInt16[SS, (ushort)(BP + 0x6 + 2)];
    CheckExternalEvents(cs14, 0x3D4D);
    // PUSH word ptr [BP + 0xc] (19EF_3D4A / 0x1DC3A)
    Stack.Push16(UInt16[SS, (ushort)(BP + 0xC)]);
    CheckExternalEvents(cs14, 0x3D50);
    // PUSH word ptr [BP + 0xa] (19EF_3D4D / 0x1DC3D)
    Stack.Push16(UInt16[SS, (ushort)(BP + 0xA)]);
    CheckExternalEvents(cs14, 0x3D54);
    // PUSH word ptr ES:[BX + 0x2] (19EF_3D50 / 0x1DC40)
    Stack.Push16(UInt16[ES, (ushort)(BX + 0x2)]);
    CheckExternalEvents(cs14, 0x3D57);
    // PUSH word ptr ES:[BX] (19EF_3D54 / 0x1DC44)
    Stack.Push16(UInt16[ES, (ushort)(BX)]);
    CheckExternalEvents(cs14, 0x3D5C);
    // CALLF 0x1000:dd52 (19EF_3D57 / 0x1DC47)
    FarCall(cs14, 0x3D5C, unknown_19EF_3E62_1DD52);
    CheckExternalEvents(cs14, 0x3D5F);
    // LES BX,[BP + 0x6] (19EF_3D5C / 0x1DC4C)
    BX = UInt16[SS, (ushort)(BP + 0x6)];
    ES = UInt16[SS, (ushort)(BP + 0x6 + 2)];
    CheckExternalEvents(cs14, 0x3D63);
    // MOV word ptr ES:[BX + 0x2],DX (19EF_3D5F / 0x1DC4F)
    UInt16[ES, (ushort)(BX + 0x2)] = DX;
    CheckExternalEvents(cs14, 0x3D66);
    // MOV word ptr ES:[BX],AX (19EF_3D63 / 0x1DC53)
    UInt16[ES, (ushort)(BX)] = AX;
    CheckExternalEvents(cs14, 0x3D68);
    // MOV SP,BP (19EF_3D66 / 0x1DC56)
    SP = BP;
    CheckExternalEvents(cs14, 0x3D69);
    // POP BP (19EF_3D68 / 0x1DC58)
    BP = Stack.Pop16();;
    CheckExternalEvents(cs14, 0x3D6C);
  ...
  public virtual Action unknown_19EF_3D92_1DC82(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3DDA);
    // MOV BX,AX (19EF_3DD8 / 0x1DCC8)
    BX = AX;
    CheckExternalEvents(cs14, 0x3DDD);
    // MOV AX,word ptr [BP + 0x6] (19EF_3DDA / 0x1DCCA)
    AX = UInt16[SS, (ushort)(BP + 0x6)];
    CheckExternalEvents(cs14, 0x3DDF);
    // DIV CX (19EF_3DDD / 0x1DCCD)
    uint op1Div19EF_3DDD = (uint)(DX << 16 | AX);
    ...
    CheckExternalEvents(cs14, 0x3DFE);
    // MOV SI,AX (19EF_3DFC / 0x1DCEC)
    SI = AX;
    CheckExternalEvents(cs14, 0x3E01);
  ...
  public virtual Action unknown_19EF_3E62_1DD52(int loadOffset) {
    ...
    CheckExternalEvents(cs14, 0x3E63);
    // PUSH BP (19EF_3E62 / 0x1DD52)
    Stack.Push16(BP);
    CheckExternalEvents(cs14, 0x3E65);
    // MOV BP,SP (19EF_3E63 / 0x1DD53)
    BP = SP;
    CheckExternalEvents(cs14, 0x3E66);
    // PUSH BX (19EF_3E65 / 0x1DD55)
    Stack.Push16(BX);
    CheckExternalEvents(cs14, 0x3E67);
    // PUSH SI (19EF_3E66 / 0x1DD56)
    Stack.Push16(SI);
    CheckExternalEvents(cs14, 0x3E6A);
    // MOV AX,word ptr [BP + 0xc] (19EF_3E67 / 0x1DD57)
    AX = UInt16[SS, (ushort)(BP + 0xC)];
    CheckExternalEvents(cs14, 0x3E6C);
    // OR AX,AX (19EF_3E6A / 0x1DD5A)
    // AX |= AX;
    AX = Alu16.Or(AX, AX);
    CheckExternalEvents(cs14, 0x3E6E);
    // JNZ 0x1000:dd73 (19EF_3E6C / 0x1DD5C)
    if(!ZeroFlag) {
      goto label_19EF_3E83_1DD73;
    }
    CheckExternalEvents(cs14, 0x3E71);
    // MOV CX,word ptr [BP + 0xa] (19EF_3E6E / 0x1DD5E)
    CX = UInt16[SS, (ushort)(BP + 0xA)];
    CheckExternalEvents(cs14, 0x3E74);
    // MOV AX,word ptr [BP + 0x8] (19EF_3E71 / 0x1DD61)
    AX = UInt16[SS, (ushort)(BP + 0x8)];
    CheckExternalEvents(cs14, 0x3E76);
    // XOR DX,DX (19EF_3E74 / 0x1DD64)
    DX = 0;
    CheckExternalEvents(cs14, 0x3E78);
    ...
    CheckExternalEvents(cs14, 0x3E7A);
    // MOV BX,AX (19EF_3E78 / 0x1DD68)
    BX = AX;
    CheckExternalEvents(cs14, 0x3E7D);
    // MOV AX,word ptr [BP + 0x6] (19EF_3E7A / 0x1DD6A)
    AX = UInt16[SS, (ushort)(BP + 0x6)];
    CheckExternalEvents(cs14, 0x3E7F);
    ...
    CheckExternalEvents(cs14, 0x3E9E);
    // MOV SI,AX (19EF_3E9C / 0x1DD8C)
    SI = AX;
    CheckExternalEvents(cs14, 0x3EA1);
  ...
  public virtual Action split_1000_E6D1_1E6D1(int loadOffset) {
    ...
    CheckExternalEvents(cs8, 0xE6D2);
    ...
    CX = Stack.Pop16();;
    AX = Stack.Pop16();;
    CheckExternalEvents(cs8, 0xE6D3);
    // INSW ES:DI,DX (1000_E6D2 / 0x1E6D2)
    UInt16[ES, (ushort)(DI)] = Cpu.In16(DX);
    DI = (ushort)(DI + Direction16);
    CheckExternalEvents(cs8, 0xE6D4);
    // POP SI (1000_E6D3 / 0x1E6D3)
    SI = Stack.Pop16();;
    CheckExternalEvents(cs8, 0xE6D5);
    // POP SI (1000_E6D4 / 0x1E6D4)
    SI = Stack.Pop16();;
    CheckExternalEvents(cs8, 0xE6D7);
    // AAD 0x45 (1000_E6D5 / 0x1E6D5)
    new Instructions8(Cpu.State, Cpu, Memory, new(Memory, Cpu, Cpu.State)).Aad(0x45);
    CheckExternalEvents(cs8, 0xE6D9);
    // DIV CL (1000_E6D7 / 0x1E6D7)
    ushort op1Div1000_E6D7 = (ushort)AX;
    ...
    CheckExternalEvents(cs8, 0xE6DA);
  ...
  public virtual Action split_1000_E903_1E903(int loadOffset) {
    ...
    CheckExternalEvents(cs8, 0xE95A);
    // PUSH ES (1000_E958 / 0x1E958)
    Stack.Push16(ES);
    CheckExternalEvents(cs8, 0xE95C);
  ...
  public virtual Action ghidra_guess_1000_FA7E_1FA7E(int loadOffset) {
    ...
    CheckExternalEvents(cs8, 0xFA7F);
    // INC DX (1000_FA7E / 0x1FA7E)
    DX = Alu16.Inc(DX);
    CheckExternalEvents(cs8, 0xFA83);
    // MOV SS,word ptr [BX + SI + 0x9090] (1000_FA7F / 0x1FA7F)
    SS = UInt16[DS, (ushort)(BX + SI + 0x9090)];
    CheckExternalEvents(cs8, 0xFA84);
    // NOP  (1000_FA83 / 0x1FA83)
    
    CheckExternalEvents(cs8, 0xFA85);
    // XCHG AX,DX (1000_FA84 / 0x1FA84)
    (DX, AX) = (AX, DX);
    CheckExternalEvents(cs8, 0xFA86);
    // INC DX (1000_FA85 / 0x1FA85)
    DX++;
    CheckExternalEvents(cs8, 0xFA87);
    // INC DX (1000_FA86 / 0x1FA86)
    DX = Alu16.Inc(DX);
    CheckExternalEvents(cs8, 0xFA89);
    // MOV DL,0xb7 (1000_FA87 / 0x1FA87)
    DL = 0xB7;
    CheckExternalEvents(cs8, 0xFA8D);
    // MOV SS,word ptr [BP + SI + 0xb7b7] (1000_FA89 / 0x1FA89)
    SS = UInt16[SS, (ushort)(BP + SI + 0xB7B7)];
    CheckExternalEvents(cs8, 0xFA8E);
    // INC DX (1000_FA8D / 0x1FA8D)
    DX++;
    CheckExternalEvents(cs8, 0xFA8F);
    // INC DX (1000_FA8E / 0x1FA8E)
    DX++;
    CheckExternalEvents(cs8, 0xFA90);
    // INC DX (1000_FA8F / 0x1FA8F)
    DX = Alu16.Inc(DX);
    CheckExternalEvents(cs8, 0xFA94);
    // MOV SS,word ptr [BX + SI + 0x9090] (1000_FA90 / 0x1FA90)
    SS = UInt16[DS, (ushort)(BX + SI + 0x9090)];
    CheckExternalEvents(cs8, 0xFA95);
    // NOP  (1000_FA94 / 0x1FA94)
    
    CheckExternalEvents(cs8, 0xFA96);
    // INC DX (1000_FA95 / 0x1FA95)
    DX++;
    CheckExternalEvents(cs8, 0xFA97);
    // INC DX (1000_FA96 / 0x1FA96)
    DX++;
    CheckExternalEvents(cs8, 0xFA98);
    // INC DX (1000_FA97 / 0x1FA97)
    DX = Alu16.Inc(DX);
    CheckExternalEvents(cs8, 0xFA9A);
    // MOV DL,0xb7 (1000_FA98 / 0x1FA98)
    DL = 0xB7;
    CheckExternalEvents(cs8, 0xFA9C);
    // MOV BH,0xb7 (1000_FA9A / 0x1FA9A)
    BH = 0xB7;
    CheckExternalEvents(cs8, 0xFA9E);
    // MOV BH,0x87 (1000_FA9C / 0x1FA9C)
    BH = 0x87;
    CheckExternalEvents(cs8, 0xFAA2);
    // MOV DX,word ptr [BP + SI + 0xb4b4] (1000_FA9E / 0x1FA9E)
    DX = UInt16[SS, (ushort)(BP + SI + 0xB4B4)];
    CheckExternalEvents(cs8, 0xFAA4);
    // JNO 0x1000:fae7 (1000_FAA2 / 0x1FAA2)
    if(!OverflowFlag) {
      goto label_1000_FAE7_1FAE7;
    }
    CheckExternalEvents(cs8, 0xFAA5);
    // INC CX (1000_FAA4 / 0x1FAA4)
    CX = Alu16.Inc(CX);
    CheckExternalEvents(cs8, 0xFAA7);
    // MOV AH,0x91 (1000_FAA5 / 0x1FAA5)
    AH = 0x91;
    CheckExternalEvents(cs8, 0xFAAA);
    // TEST AX,0xb4b5 (1000_FAA7 / 0x1FAA7)
    Alu16.And(AX, 0xB4B5);
    CheckExternalEvents(cs8, 0xFAAE);
    // IMUL AX,word ptr [BP + DI + 0x41],-0x4c (1000_FAAA / 0x1FAAA)
    unchecked {
      AX = (ushort)(Alu16.Imul((short)UInt16[SS, (ushort)(BP + DI + 0x41)], (short)0xFFB4));
    }
    CheckExternalEvents(cs8, 0xFAAF);
    // XCHG AX,CX (1000_FAAE / 0x1FAAE)
    (CX, AX) = (AX, CX);
    CheckExternalEvents(cs8, 0xFAB1);
    // MOV AH,0xa9 (1000_FAAF / 0x1FAAF)
    AH = 0xA9;
    CheckExternalEvents(cs8, 0xFAB3);
    // MOV CH,0x71 (1000_FAB1 / 0x1FAB1)
    CH = 0x71;
    CheckExternalEvents(cs8, 0xFAB4);
    // INC CX (1000_FAB3 / 0x1FAB3)
    CX++;
    CheckExternalEvents(cs8, 0xFAB5);
    // INC AX (1000_FAB4 / 0x1FAB4)
    AX = Alu16.Inc(AX);
    CheckExternalEvents(cs8, 0xFAB7);
    // MOV AH,0x91 (1000_FAB5 / 0x1FAB5)
    AH = 0x91;
    CheckExternalEvents(cs8, 0xFABB);
    // MOV word ptr [DI + 0x6b42],CS (1000_FAB7 / 0x1FAB7)
    UInt16[DS, (ushort)(DI + 0x6B42)] = cs8;
    CheckExternalEvents(cs8, 0xFABC);
    // INC DX (1000_FABB / 0x1FABB)
    DX++;
    CheckExternalEvents(cs8, 0xFABD);
    // INC CX (1000_FABC / 0x1FABC)
    CX = Alu16.Inc(CX);
    CheckExternalEvents(cs8, 0xFABF);
    // MOV AH,0x91 (1000_FABD / 0x1FABD)
    AH = 0x91;
    CheckExternalEvents(cs8, 0xFAC0);
    // XCHG AX,DX (1000_FABF / 0x1FABF)
    (DX, AX) = (AX, DX);
    CheckExternalEvents(cs8, 0xFAC2);
    // MOV AH,0xb3 (1000_FAC0 / 0x1FAC0)
    AH = 0xB3;
    CheckExternalEvents(cs8, 0xFAC4);
    // JNO 0x1000:fb06 (1000_FAC2 / 0x1FAC2)
    if(!OverflowFlag) {
      goto label_1000_FB06_1FB06;
    }
    CheckExternalEvents(cs8, 0xFAC5);
    // INC CX (1000_FAC4 / 0x1FAC4)
    CX = Alu16.Inc(CX);
    CheckExternalEvents(cs8, 0xFAC7);
    // MOV CH,0x91 (1000_FAC5 / 0x1FAC5)
    CH = 0x91;
    CheckExternalEvents(cs8, 0xFACA);
    // TEST AX,0xb4b5 (1000_FAC7 / 0x1FAC7)
    Alu16.And(AX, 0xB4B5);
    CheckExternalEvents(cs8, 0xFACE);
    // IMUL AX,word ptr [BX + DI + 0x40],-0x70 (1000_FACA / 0x1FACA)
    unchecked {
      AX = (ushort)(Alu16.Imul((short)UInt16[DS, (ushort)(BX + DI + 0x40)], (short)0xFF90));
    }
    CheckExternalEvents(cs8, 0xFACF);
    // XCHG AX,DX (1000_FACE / 0x1FACE)
    (DX, AX) = (AX, DX);
    CheckExternalEvents(cs8, 0xFAD1);
    // MOV AH,0xa9 (1000_FACF / 0x1FACF)
    AH = 0xA9;
    CheckExternalEvents(cs8, 0xFAD3);
    // MOV CH,0x71 (1000_FAD1 / 0x1FAD1)
    CH = 0x71;
    CheckExternalEvents(cs8, 0xFAD4);
    // INC CX (1000_FAD3 / 0x1FAD3)
    CX++;
    CheckExternalEvents(cs8, 0xFAD5);
    // INC AX (1000_FAD4 / 0x1FAD4)
    AX = Alu16.Inc(AX);
    CheckExternalEvents(cs8, 0xFAD7);
    // MOV BH,0xb7 (1000_FAD5 / 0x1FAD5)
    BH = 0xB7;
    CheckExternalEvents(cs8, 0xFAD9);
    // MOV CH,0xb4 (1000_FAD7 / 0x1FAD7)
    CH = 0xB4;
    CheckExternalEvents(cs8, 0xFADA);
    // INC DX (1000_FAD9 / 0x1FAD9)
    DX++;
    CheckExternalEvents(cs8, 0xFADE);
    // IMUL AX,word ptr [BX + DI + 0x40],0x41 (1000_FADA / 0x1FADA)
    unchecked {
      AX = (ushort)(Alu16.Imul((short)UInt16[DS, (ushort)(BX + DI + 0x40)], (short)0x41));
    }
    CheckExternalEvents(cs8, 0xFADF);
    // INC DX (1000_FADE / 0x1FADE)
    DX++;
    CheckExternalEvents(cs8, 0xFAE0);
    // INC AX (1000_FADF / 0x1FADF)
    AX++;
    CheckExternalEvents(cs8, 0xFAE1);
    // INC AX (1000_FAE0 / 0x1FAE0)
    AX++;
    CheckExternalEvents(cs8, 0xFAE2);
    // INC BX (1000_FAE1 / 0x1FAE1)
    BX++;
    CheckExternalEvents(cs8, 0xFAE3);
    // INC BX (1000_FAE2 / 0x1FAE2)
    BX++;
    CheckExternalEvents(cs8, 0xFAE4);
    // INC BX (1000_FAE3 / 0x1FAE3)
    BX++;
    CheckExternalEvents(cs8, 0xFAE5);
    // INC BX (1000_FAE4 / 0x1FAE4)
    BX++;
    CheckExternalEvents(cs8, 0xFAE6);
    // INC AX (1000_FAE5 / 0x1FAE5)
    AX++;
    CheckExternalEvents(cs8, 0xFAE7);
    // INC DX (1000_FAE6 / 0x1FAE6)
    DX++;
    label_1000_FAE7_1FAE7:
    CheckExternalEvents(cs8, 0xFAE8);
    // INC AX (1000_FAE7 / 0x1FAE7)
    AX++;
    CheckExternalEvents(cs8, 0xFAE9);
    // INC AX (1000_FAE8 / 0x1FAE8)
    AX++;
    CheckExternalEvents(cs8, 0xFAEA);
    // INC BX (1000_FAE9 / 0x1FAE9)
    BX++;
    CheckExternalEvents(cs8, 0xFAEB);
    // INC BX (1000_FAEA / 0x1FAEA)
    BX++;
    CheckExternalEvents(cs8, 0xFAEC);
    // INC CX (1000_FAEB / 0x1FAEB)
    CX++;
    CheckExternalEvents(cs8, 0xFAED);
    // INC CX (1000_FAEC / 0x1FAEC)
    CX++;
    CheckExternalEvents(cs8, 0xFAEE);
    // INC AX (1000_FAED / 0x1FAED)
    AX++;
    CheckExternalEvents(cs8, 0xFAEF);
    // INC DX (1000_FAEE / 0x1FAEE)
    DX++;
    CheckExternalEvents(cs8, 0xFAF0);
    // INC DX (1000_FAEF / 0x1FAEF)
    DX++;
    CheckExternalEvents(cs8, 0xFAF1);
    // INC AX (1000_FAF0 / 0x1FAF0)
    AX++;
    CheckExternalEvents(cs8, 0xFAF2);
    // INC BX (1000_FAF1 / 0x1FAF1)
    BX++;
    CheckExternalEvents(cs8, 0xFAF3);
    // INC BX (1000_FAF2 / 0x1FAF2)
    BX++;
    CheckExternalEvents(cs8, 0xFAF4);
    // INC CX (1000_FAF3 / 0x1FAF3)
    CX++;
    CheckExternalEvents(cs8, 0xFAF5);
    // INC BX (1000_FAF4 / 0x1FAF4)
    BX++;
    CheckExternalEvents(cs8, 0xFAF6);
    // INC CX (1000_FAF5 / 0x1FAF5)
    CX++;
    CheckExternalEvents(cs8, 0xFAF7);
    // INC CX (1000_FAF6 / 0x1FAF6)
    CX++;
    CheckExternalEvents(cs8, 0xFAF8);
    // INC CX (1000_FAF7 / 0x1FAF7)
    CX++;
    CheckExternalEvents(cs8, 0xFAF9);
    // INC DX (1000_FAF8 / 0x1FAF8)
    DX++;
    CheckExternalEvents(cs8, 0xFAFA);
    // INC DX (1000_FAF9 / 0x1FAF9)
    DX++;
    CheckExternalEvents(cs8, 0xFAFB);
    // INC CX (1000_FAFA / 0x1FAFA)
    CX++;
    CheckExternalEvents(cs8, 0xFAFC);
    // INC BX (1000_FAFB / 0x1FAFB)
    BX++;
    CheckExternalEvents(cs8, 0xFAFD);
    // INC AX (1000_FAFC / 0x1FAFC)
    AX++;
    CheckExternalEvents(cs8, 0xFAFE);
    // INC AX (1000_FAFD / 0x1FAFD)
    AX++;
    CheckExternalEvents(cs8, 0xFAFF);
    // INC AX (1000_FAFE / 0x1FAFE)
    AX++;
    CheckExternalEvents(cs8, 0xFB00);
    // INC AX (1000_FAFF / 0x1FAFF)
    AX++;
    CheckExternalEvents(cs8, 0xFB01);
    // INC AX (1000_FB00 / 0x1FB00)
    AX++;
    CheckExternalEvents(cs8, 0xFB02);
    // INC AX (1000_FB01 / 0x1FB01)
    AX++;
    CheckExternalEvents(cs8, 0xFB03);
    // INC DX (1000_FB02 / 0x1FB02)
    DX++;
    CheckExternalEvents(cs8, 0xFB04);
    // INC AX (1000_FB03 / 0x1FB03)
    AX++;
    CheckExternalEvents(cs8, 0xFB05);
    // INC AX (1000_FB04 / 0x1FB04)
    AX++;
    CheckExternalEvents(cs8, 0xFB06);
    // INC AX (1000_FB05 / 0x1FB05)
    AX++;
    label_1000_FB06_1FB06:
    CheckExternalEvents(cs8, 0xFB07);
    // INC CX (1000_FB06 / 0x1FB06)
    CX++;
    CheckExternalEvents(cs8, 0xFB08);
    // INC CX (1000_FB07 / 0x1FB07)
    CX++;
    CheckExternalEvents(cs8, 0xFB0A);
    // XOR AL,0x42 (1000_FB08 / 0x1FB08)
    AL ^= 0x42;
    CheckExternalEvents(cs8, 0xFB0B);
    // INC DX (1000_FB0A / 0x1FB0A)
    DX++;
    CheckExternalEvents(cs8, 0xFB0C);
    // INC CX (1000_FB0B / 0x1FB0B)
    CX++;
    CheckExternalEvents(cs8, 0xFB0D);
    // INC CX (1000_FB0C / 0x1FB0C)
    CX++;
    CheckExternalEvents(cs8, 0xFB0E);
    // INC CX (1000_FB0D / 0x1FB0D)
    CX++;
    CheckExternalEvents(cs8, 0xFB0F);
    // INC DX (1000_FB0E / 0x1FB0E)
    DX++;
    CheckExternalEvents(cs8, 0xFB12);
    // CMP DI,word ptr SS:[BX + SI] (1000_FB0F / 0x1FB0F)
    Alu16.Sub(DI, UInt16[SS, (ushort)(BX + SI)]);
    CheckExternalEvents(cs8, 0xFB15);
    // XOR byte ptr [BX + DI + 0x41],AL (1000_FB12 / 0x1FB12)
    UInt8[DS, (ushort)(BX + DI + 0x41)] ^= AL;
    CheckExternalEvents(cs8, 0xFB16);
    // INC CX (1000_FB15 / 0x1FB15)
    CX++;
    CheckExternalEvents(cs8, 0xFB18);
    // XOR byte ptr [DI],DH (1000_FB16 / 0x1FB16)
    // UInt8[DS, (ushort)(DI)] ^= DH;
    UInt8[DS, (ushort)(DI)] = Alu8.Xor(UInt8[DS, (ushort)(DI)], DH);
    CheckExternalEvents(cs8, 0xFB1A);
    // JO 0x1000:fb56 (1000_FB18 / 0x1FB18)
    if(OverflowFlag) {
      goto label_1000_FB56_1FB56;
    }
    CheckExternalEvents(cs8, 0xFB1D);
    // XOR byte ptr [BX + DI + 0x41],AL (1000_FB1A / 0x1FB1A)
    // UInt8[DS, (ushort)(BX + DI + 0x41)] ^= AL;
    UInt8[DS, (ushort)(BX + DI + 0x41)] = Alu8.Xor(UInt8[DS, (ushort)(BX + DI + 0x41)], AL);
    CheckExternalEvents(cs8, 0xFB1E);
    // OUTSW DX,SI (1000_FB1D / 0x1FB1D)
    Cpu.Out16(DX, UInt16[DS, (ushort)(SI)]);
    SI = (ushort)(SI + Direction16);
    CheckExternalEvents(cs8, 0xFB20);
    // PUSH 0x70 (1000_FB1E / 0x1FB1E)
    Stack.Push16(0x70);
    CheckExternalEvents(cs8, 0xFB21);
    // INC CX (1000_FB20 / 0x1FB20)
    CX++;
    CheckExternalEvents(cs8, 0xFB22);
    // INC AX (1000_FB21 / 0x1FB21)
    AX = Alu16.Inc(AX);
    CheckExternalEvents(cs8, 0xFB24);
    ...
    CheckExternalEvents(cs8, 0xFB26);
    // JNP 0x1000:fb69 (1000_FB24 / 0x1FB24)
    if(!ParityFlag) {
      goto label_1000_FB69_1FB69;
    }
    CheckExternalEvents(cs8, 0xFB27);
    // INC CX (1000_FB26 / 0x1FB26)
    CX++;
    CheckExternalEvents(cs8, 0xFB28);
    // INC CX (1000_FB27 / 0x1FB27)
    CX++;
    CheckExternalEvents(cs8, 0xFB29);
    // INC AX (1000_FB28 / 0x1FB28)
    AX++;
    CheckExternalEvents(cs8, 0xFB2A);
    // INC AX (1000_FB29 / 0x1FB29)
    AX = Alu16.Inc(AX);
    CheckExternalEvents(cs8, 0xFB2C);
    // JNS 0x1000:fba5 (1000_FB2A / 0x1FB2A)
    if(!SignFlag) {
      goto label_1000_FBA5_1FBA5;
    }
    CheckExternalEvents(cs8, 0xFB2E);
    // JNP 0x1000:fb6f (1000_FB2C / 0x1FB2C)
    if(!ParityFlag) {
      goto label_1000_FB6F_1FB6F;
    }
    CheckExternalEvents(cs8, 0xFB2F);
    // INC CX (1000_FB2E / 0x1FB2E)
    CX++;
    CheckExternalEvents(cs8, 0xFB30);
    // INC AX (1000_FB2F / 0x1FB2F)
    AX++;
    CheckExternalEvents(cs8, 0xFB31);
    // INC BX (1000_FB30 / 0x1FB30)
    BX++;
    CheckExternalEvents(cs8, 0xFB32);
    // INC AX (1000_FB31 / 0x1FB31)
    AX = Alu16.Inc(AX);
    CheckExternalEvents(cs8, 0xFB34);
    // JL 0x1000:fba8 (1000_FB32 / 0x1FB32)
    if(SignFlag != OverflowFlag) {
      goto label_1000_FBA8_1FBA8;
    }
    CheckExternalEvents(cs8, 0xFB36);
    // JLE 0x1000:fb76 (1000_FB34 / 0x1FB34)
    if(ZeroFlag || SignFlag != OverflowFlag) {
      goto label_1000_FB76_1FB76;
    }
    CheckExternalEvents(cs8, 0xFB37);
    // INC AX (1000_FB36 / 0x1FB36)
    AX++;
    CheckExternalEvents(cs8, 0xFB38);
    // INC AX (1000_FB37 / 0x1FB37)
    AX++;
    CheckExternalEvents(cs8, 0xFB39);
    // INC AX (1000_FB38 / 0x1FB38)
    AX++;
    CheckExternalEvents(cs8, 0xFB3A);
    // INC BX (1000_FB39 / 0x1FB39)
    BX++;
    CheckExternalEvents(cs8, 0xFB3B);
    // INC BX (1000_FB3A / 0x1FB3A)
    BX++;
    CheckExternalEvents(cs8, 0xFB3C);
    // INC CX (1000_FB3B / 0x1FB3B)
    CX++;
    CheckExternalEvents(cs8, 0xFB3D);
    // INC CX (1000_FB3C / 0x1FB3C)
    CX++;
    CheckExternalEvents(cs8, 0xFB3E);
    // INC AX (1000_FB3D / 0x1FB3D)
    AX++;
    CheckExternalEvents(cs8, 0xFB3F);
    // INC AX (1000_FB3E / 0x1FB3E)
    AX++;
    CheckExternalEvents(cs8, 0xFB40);
    // INC AX (1000_FB3F / 0x1FB3F)
    AX++;
    CheckExternalEvents(cs8, 0xFB41);
    // INC AX (1000_FB40 / 0x1FB40)
    AX++;
    CheckExternalEvents(cs8, 0xFB42);
    // INC AX (1000_FB41 / 0x1FB41)
    AX++;
    CheckExternalEvents(cs8, 0xFB43);
    // INC DX (1000_FB42 / 0x1FB42)
    DX++;
    CheckExternalEvents(cs8, 0xFB44);
    // INC CX (1000_FB43 / 0x1FB43)
    CX++;
    CheckExternalEvents(cs8, 0xFB45);
    // INC CX (1000_FB44 / 0x1FB44)
    CX++;
    CheckExternalEvents(cs8, 0xFB46);
    // INC CX (1000_FB45 / 0x1FB45)
    CX++;
    CheckExternalEvents(cs8, 0xFB47);
    // INC DX (1000_FB46 / 0x1FB46)
    DX++;
    CheckExternalEvents(cs8, 0xFB48);
    // INC DX (1000_FB47 / 0x1FB47)
    DX++;
    CheckExternalEvents(cs8, 0xFB4A);
    // XOR AL,0x42 (1000_FB48 / 0x1FB48)
    AL ^= 0x42;
    CheckExternalEvents(cs8, 0xFB4B);
    // INC CX (1000_FB4A / 0x1FB4A)
    CX++;
    CheckExternalEvents(cs8, 0xFB4C);
    // INC DX (1000_FB4B / 0x1FB4B)
    DX++;
    CheckExternalEvents(cs8, 0xFB4D);
    // INC DX (1000_FB4C / 0x1FB4C)
    DX++;
    CheckExternalEvents(cs8, 0xFB4E);
    // INC CX (1000_FB4D / 0x1FB4D)
    CX++;
    CheckExternalEvents(cs8, 0xFB4F);
    // INC DX (1000_FB4E / 0x1FB4E)
    DX++;
    CheckExternalEvents(cs8, 0xFB53);
    // CMP word ptr SS:[BP + SI + 0x41],AX (1000_FB4F / 0x1FB4F)
    Alu16.Sub(UInt16[SS, (ushort)(BP + SI + 0x41)], AX);
    CheckExternalEvents(cs8, 0xFB54);
    // INC DX (1000_FB53 / 0x1FB53)
    DX++;
    CheckExternalEvents(cs8, 0xFB55);
    // INC DX (1000_FB54 / 0x1FB54)
    DX++;
    CheckExternalEvents(cs8, 0xFB56);
    // INC CX (1000_FB55 / 0x1FB55)
    CX++;
    label_1000_FB56_1FB56:
    CheckExternalEvents(cs8, 0xFB58);
    // XOR BH,byte ptr [BX] (1000_FB56 / 0x1FB56)
    BH ^= UInt8[DS, (ushort)(BX)];
    CheckExternalEvents(cs8, 0xFB5B);
    // CMP byte ptr [BP + SI + 0x42],AL (1000_FB58 / 0x1FB58)
    Alu8.Sub(UInt8[SS, (ushort)(BP + SI + 0x42)], AL);
    CheckExternalEvents(cs8, 0xFB5C);
    // INC DX (1000_FB5B / 0x1FB5B)
    DX++;
    CheckExternalEvents(cs8, 0xFB5D);
    // INC DX (1000_FB5C / 0x1FB5C)
    DX++;
    CheckExternalEvents(cs8, 0xFB5E);
    // INC DX (1000_FB5D / 0x1FB5D)
    DX++;
    CheckExternalEvents(cs8, 0xFB5F);
    // INC DX (1000_FB5E / 0x1FB5E)
    DX = Alu16.Inc(DX);
    CheckExternalEvents(cs8, 0xFB61);
    // JO 0x1000:fba3 (1000_FB5F / 0x1FB5F)
    if(OverflowFlag) {
      goto label_1000_FBA3_1FBA3;
    }
    CheckExternalEvents(cs8, 0xFB62);
    // INC DX (1000_FB61 / 0x1FB61)
    DX++;
    CheckExternalEvents(cs8, 0xFB63);
    // INC DX (1000_FB62 / 0x1FB62)
    DX++;
    CheckExternalEvents(cs8, 0xFB64);
    // INC CX (1000_FB63 / 0x1FB63)
    CX++;
    CheckExternalEvents(cs8, 0xFB65);
    // INC CX (1000_FB64 / 0x1FB64)
    CX++;
    CheckExternalEvents(cs8, 0xFB66);
    // INC CX (1000_FB65 / 0x1FB65)
    CX++;
    CheckExternalEvents(cs8, 0xFB67);
    // INC DX (1000_FB66 / 0x1FB66)
    DX++;
    CheckExternalEvents(cs8, 0xFB68);
    // INC DX (1000_FB67 / 0x1FB67)
    DX++;
    CheckExternalEvents(cs8, 0xFB69);
    // INC DX (1000_FB68 / 0x1FB68)
    DX++;
    label_1000_FB69_1FB69:
    CheckExternalEvents(cs8, 0xFB6A);
    // INC CX (1000_FB69 / 0x1FB69)
    CX++;
    CheckExternalEvents(cs8, 0xFB6B);
    // INC CX (1000_FB6A / 0x1FB6A)
    CX++;
    CheckExternalEvents(cs8, 0xFB6C);
    // INC DX (1000_FB6B / 0x1FB6B)
    DX++;
    CheckExternalEvents(cs8, 0xFB6D);
    // INC CX (1000_FB6C / 0x1FB6C)
    CX++;
    CheckExternalEvents(cs8, 0xFB6E);
    // INC DX (1000_FB6D / 0x1FB6D)
    DX++;
    CheckExternalEvents(cs8, 0xFB6F);
    // INC DX (1000_FB6E / 0x1FB6E)
    DX++;
    label_1000_FB6F_1FB6F:
    CheckExternalEvents(cs8, 0xFB70);
    // INC DX (1000_FB6F / 0x1FB6F)
    DX++;
    CheckExternalEvents(cs8, 0xFB71);
    // INC DX (1000_FB70 / 0x1FB70)
    DX++;
    CheckExternalEvents(cs8, 0xFB72);
    // INC DX (1000_FB71 / 0x1FB71)
    DX++;
    CheckExternalEvents(cs8, 0xFB73);
    // INC DX (1000_FB72 / 0x1FB72)
    DX++;
    CheckExternalEvents(cs8, 0xFB74);
    // INC DX (1000_FB73 / 0x1FB73)
    DX++;
    CheckExternalEvents(cs8, 0xFB75);
    // INC CX (1000_FB74 / 0x1FB74)
    CX++;
    CheckExternalEvents(cs8, 0xFB76);
    // INC CX (1000_FB75 / 0x1FB75)
    CX++;
    label_1000_FB76_1FB76:
    CheckExternalEvents(cs8, 0xFB77);
    // INC CX (1000_FB76 / 0x1FB76)
    CX++;
    CheckExternalEvents(cs8, 0xFB78);
    // INC CX (1000_FB77 / 0x1FB77)
    CX++;
    CheckExternalEvents(cs8, 0xFB79);
    // INC DX (1000_FB78 / 0x1FB78)
    DX++;
    CheckExternalEvents(cs8, 0xFB7A);
    // INC DX (1000_FB79 / 0x1FB79)
    DX++;
    label_1000_FB7A_1FB7A:
    CheckExternalEvents(cs8, 0xFB7B);
    // INC DX (1000_FB7A / 0x1FB7A)
    DX++;
    CheckExternalEvents(cs8, 0xFB7C);
    // INC DX (1000_FB7B / 0x1FB7B)
    DX++;
    CheckExternalEvents(cs8, 0xFB7D);
    // INC DX (1000_FB7C / 0x1FB7C)
    DX++;
    CheckExternalEvents(cs8, 0xFB7E);
    // INC CX (1000_FB7D / 0x1FB7D)
    CX++;
    CheckExternalEvents(cs8, 0xFB7F);
    // INC CX (1000_FB7E / 0x1FB7E)
    CX++;
    CheckExternalEvents(cs8, 0xFB80);
    // INC CX (1000_FB7F / 0x1FB7F)
    CX++;
    CheckExternalEvents(cs8, 0xFB81);
    // INC CX (1000_FB80 / 0x1FB80)
    CX++;
    CheckExternalEvents(cs8, 0xFB82);
    // INC CX (1000_FB81 / 0x1FB81)
    CX++;
    CheckExternalEvents(cs8, 0xFB83);
    // INC DX (1000_FB82 / 0x1FB82)
    DX++;
    CheckExternalEvents(cs8, 0xFB84);
    // INC DX (1000_FB83 / 0x1FB83)
    DX++;
    CheckExternalEvents(cs8, 0xFB85);
    // INC DX (1000_FB84 / 0x1FB84)
    DX++;
    CheckExternalEvents(cs8, 0xFB86);
    // INC DX (1000_FB85 / 0x1FB85)
    DX++;
    CheckExternalEvents(cs8, 0xFB87);
    // INC DX (1000_FB86 / 0x1FB86)
    DX++;
    CheckExternalEvents(cs8, 0xFB88);
    // INC DX (1000_FB87 / 0x1FB87)
    DX++;
    CheckExternalEvents(cs8, 0xFB89);
    // INC CX (1000_FB88 / 0x1FB88)
    CX++;
    CheckExternalEvents(cs8, 0xFB8A);
    // INC CX (1000_FB89 / 0x1FB89)
    CX++;
    label_1000_FB8A_1FB8A:
    CheckExternalEvents(cs8, 0xFB8B);
    // INC CX (1000_FB8A / 0x1FB8A)
    CX++;
    CheckExternalEvents(cs8, 0xFB8C);
    // INC DX (1000_FB8B / 0x1FB8B)
    DX++;
    CheckExternalEvents(cs8, 0xFB8D);
    // INC DX (1000_FB8C / 0x1FB8C)
    DX++;
    CheckExternalEvents(cs8, 0xFB8E);
    // INC DX (1000_FB8D / 0x1FB8D)
    DX++;
    CheckExternalEvents(cs8, 0xFB8F);
    // INC CX (1000_FB8E / 0x1FB8E)
    CX++;
    CheckExternalEvents(cs8, 0xFB90);
    // INC CX (1000_FB8F / 0x1FB8F)
    CX++;
    CheckExternalEvents(cs8, 0xFB91);
    // INC CX (1000_FB90 / 0x1FB90)
    CX++;
    CheckExternalEvents(cs8, 0xFB92);
    // INC CX (1000_FB91 / 0x1FB91)
    CX++;
    CheckExternalEvents(cs8, 0xFB94);
    // ADD CL,byte ptr [SI] (1000_FB92 / 0x1FB92)
    CL += UInt8[DS, (ushort)(SI)];
    CheckExternalEvents(cs8, 0xFB95);
    // INC CX (1000_FB94 / 0x1FB94)
    CX++;
    CheckExternalEvents(cs8, 0xFB96);
    // INC CX (1000_FB95 / 0x1FB95)
    CX++;
    CheckExternalEvents(cs8, 0xFB97);
    // INC CX (1000_FB96 / 0x1FB96)
    CX++;
    CheckExternalEvents(cs8, 0xFB98);
    // INC CX (1000_FB97 / 0x1FB97)
    CX++;
    CheckExternalEvents(cs8, 0xFB99);
    // INC CX (1000_FB98 / 0x1FB98)
    CX++;
    CheckExternalEvents(cs8, 0xFB9A);
    // INC CX (1000_FB99 / 0x1FB99)
    CX++;
    label_1000_FB9A_1FB9A:
    CheckExternalEvents(cs8, 0xFB9B);
    // INC CX (1000_FB9A / 0x1FB9A)
    CX++;
    CheckExternalEvents(cs8, 0xFB9F);
    // ADD CX,word ptr [0x4242] (1000_FB9B / 0x1FB9B)
    CX += UInt16[DS, 0x4242];
    CheckExternalEvents(cs8, 0xFBA0);
    // INC CX (1000_FB9F / 0x1FB9F)
    CX++;
    CheckExternalEvents(cs8, 0xFBA1);
    // INC CX (1000_FBA0 / 0x1FBA0)
    CX++;
    CheckExternalEvents(cs8, 0xFBA2);
    // INC DX (1000_FBA1 / 0x1FBA1)
    DX++;
    CheckExternalEvents(cs8, 0xFBA3);
    // INC DX (1000_FBA2 / 0x1FBA2)
    DX = Alu16.Inc(DX);
    label_1000_FBA3_1FBA3:
    CheckExternalEvents(cs8, 0xFBA5);
    // JNO 0x1000:fb7a (1000_FBA3 / 0x1FBA3)
    if(!OverflowFlag) {
      goto label_1000_FB7A_1FB7A;
    }
    label_1000_FBA5_1FBA5:
    CheckExternalEvents(cs8, 0xFBA6);
    // INC DX (1000_FBA5 / 0x1FBA5)
    DX++;
    CheckExternalEvents(cs8, 0xFBA7);
    // INC DX (1000_FBA6 / 0x1FBA6)
    DX++;
    CheckExternalEvents(cs8, 0xFBA8);
    // INC CX (1000_FBA7 / 0x1FBA7)
    CX++;
    label_1000_FBA8_1FBA8:
    CheckExternalEvents(cs8, 0xFBA9);
    // INC DX (1000_FBA8 / 0x1FBA8)
    DX++;
    CheckExternalEvents(cs8, 0xFBAA);
    // INC DX (1000_FBA9 / 0x1FBA9)
    DX++;
    label_1000_FBAA_1FBAA:
    CheckExternalEvents(cs8, 0xFBAB);
    // INC DX (1000_FBAA / 0x1FBAA)
    DX++;
    CheckExternalEvents(cs8, 0xFBAE);
    // IMUL DX,BP,0x41 (1000_FBAB / 0x1FBAB)
    unchecked {
      DX = (ushort)(Alu16.Imul((short)BP, (short)0x41));
    }
    CheckExternalEvents(cs8, 0xFBAF);
    // INC CX (1000_FBAE / 0x1FBAE)
    CX++;
    CheckExternalEvents(cs8, 0xFBB0);
    // INC CX (1000_FBAF / 0x1FBAF)
    CX++;
    CheckExternalEvents(cs8, 0xFBB1);
    // INC CX (1000_FBB0 / 0x1FBB0)
    CX++;
    CheckExternalEvents(cs8, 0xFBB2);
    // INC CX (1000_FBB1 / 0x1FBB1)
    CX++;
    CheckExternalEvents(cs8, 0xFBB3);
    // INC CX (1000_FBB2 / 0x1FBB2)
    CX = Alu16.Inc(CX);
    CheckExternalEvents(cs8, 0xFBB5);
    // JNO 0x1000:fb8a (1000_FBB3 / 0x1FBB3)
    if(!OverflowFlag) {
      goto label_1000_FB8A_1FB8A;
    }
    CheckExternalEvents(cs8, 0xFBB6);
    // INC CX (1000_FBB5 / 0x1FBB5)
    CX++;
    CheckExternalEvents(cs8, 0xFBB7);
    // INC CX (1000_FBB6 / 0x1FBB6)
    CX++;
    CheckExternalEvents(cs8, 0xFBB8);
    // INC CX (1000_FBB7 / 0x1FBB7)
    CX++;
    CheckExternalEvents(cs8, 0xFBB9);
    // INC CX (1000_FBB8 / 0x1FBB8)
    CX++;
    CheckExternalEvents(cs8, 0xFBBA);
    // INC DX (1000_FBB9 / 0x1FBB9)
    DX++;
    CheckExternalEvents(cs8, 0xFBBB);
    // INC DX (1000_FBBA / 0x1FBBA)
    DX++;
    CheckExternalEvents(cs8, 0xFBBE);
    // IMUL DX,BP,0x41 (1000_FBBB / 0x1FBBB)
    unchecked {
      DX = (ushort)(Alu16.Imul((short)BP, (short)0x41));
    }
    CheckExternalEvents(cs8, 0xFBBF);
    // INC CX (1000_FBBE / 0x1FBBE)
    CX++;
    CheckExternalEvents(cs8, 0xFBC0);
    // INC CX (1000_FBBF / 0x1FBBF)
    CX++;
    CheckExternalEvents(cs8, 0xFBC1);
    // INC CX (1000_FBC0 / 0x1FBC0)
    CX++;
    CheckExternalEvents(cs8, 0xFBC2);
    // INC CX (1000_FBC1 / 0x1FBC1)
    CX++;
    CheckExternalEvents(cs8, 0xFBC3);
    // INC DX (1000_FBC2 / 0x1FBC2)
    DX = Alu16.Inc(DX);
    CheckExternalEvents(cs8, 0xFBC5);
    // JNO 0x1000:fb9a (1000_FBC3 / 0x1FBC3)
    if(!OverflowFlag) {
      goto label_1000_FB9A_1FB9A;
    }
    CheckExternalEvents(cs8, 0xFBC6);
    // INC DX (1000_FBC5 / 0x1FBC5)
    DX++;
    CheckExternalEvents(cs8, 0xFBC7);
    // INC DX (1000_FBC6 / 0x1FBC6)
    DX++;
    CheckExternalEvents(cs8, 0xFBC8);
    // INC DX (1000_FBC7 / 0x1FBC7)
    DX++;
    CheckExternalEvents(cs8, 0xFBC9);
    // INC DX (1000_FBC8 / 0x1FBC8)
    DX++;
    CheckExternalEvents(cs8, 0xFBCA);
    // INC DX (1000_FBC9 / 0x1FBC9)
    DX++;
    CheckExternalEvents(cs8, 0xFBCB);
    // INC DX (1000_FBCA / 0x1FBCA)
    DX++;
    CheckExternalEvents(cs8, 0xFBCE);
    // IMUL DX,BP,0x41 (1000_FBCB / 0x1FBCB)
    unchecked {
      DX = (ushort)(Alu16.Imul((short)BP, (short)0x41));
    }
    CheckExternalEvents(cs8, 0xFBCF);
    // INC DX (1000_FBCE / 0x1FBCE)
    DX++;
    label_1000_FBCF_1FBCF:
    CheckExternalEvents(cs8, 0xFBD0);
    // INC CX (1000_FBCF / 0x1FBCF)
    CX++;
    CheckExternalEvents(cs8, 0xFBD1);
    // INC CX (1000_FBD0 / 0x1FBD0)
    CX++;
    CheckExternalEvents(cs8, 0xFBD2);
    // INC DX (1000_FBD1 / 0x1FBD1)
    DX++;
    CheckExternalEvents(cs8, 0xFBD3);
    // INC DX (1000_FBD2 / 0x1FBD2)
    DX = Alu16.Inc(DX);
    CheckExternalEvents(cs8, 0xFBD5);
    // JNO 0x1000:fbaa (1000_FBD3 / 0x1FBD3)
    if(!OverflowFlag) {
      goto label_1000_FBAA_1FBAA;
    }
    CheckExternalEvents(cs8, 0xFBD7);
    // OR AL,0x42 (1000_FBD5 / 0x1FBD5)
    AL |= 0x42;
    CheckExternalEvents(cs8, 0xFBD8);
    // INC CX (1000_FBD7 / 0x1FBD7)
    CX++;
    CheckExternalEvents(cs8, 0xFBD9);
    // INC CX (1000_FBD8 / 0x1FBD8)
    CX++;
    CheckExternalEvents(cs8, 0xFBDA);
    // INC DX (1000_FBD9 / 0x1FBD9)
    DX++;
    CheckExternalEvents(cs8, 0xFBDB);
    // INC CX (1000_FBDA / 0x1FBDA)
    CX++;
    CheckExternalEvents(cs8, 0xFBDE);
    // IMUL DX,BP,-0x2b (1000_FBDB / 0x1FBDB)
    unchecked {
      DX = (ushort)(Alu16.Imul((short)BP, (short)0xFFD5));
    }
    CheckExternalEvents(cs8, 0xFBE1);
    // SUB AL,0xe9 (1000_FBDE / 0x1FBDE)
    // AL -= 0xE9;
    AL = Alu8.Sub(AL, 0xE9);
    CheckExternalEvents(cs8, 0xFBE3);
    // LOOPNZ 0x1000:fbcf (1000_FBE1 / 0x1FBE1)
    if(--CX != 0 && !ZeroFlag) {
      goto label_1000_FBCF_1FBCF;
    }
    CheckExternalEvents(cs8, 0xFBE4);
    // IN AL,DX (1000_FBE3 / 0x1FBE3)
    AL = Cpu.In8(DX);
    CheckExternalEvents(cs8, 0xFBE5);
    // STC  (1000_FBE4 / 0x1FBE4)
    CarryFlag = true;
    CheckExternalEvents(cs8, 0xFBE7);
    // AAD 0x23 (1000_FBE5 / 0x1FBE5)
    new Instructions8(Cpu.State, Cpu, Memory, new(Memory, Cpu, Cpu.State)).Aad(0x23);
    CheckExternalEvents(cs8, 0xFBE9);
    // AND BP,SI (1000_FBE7 / 0x1FBE7)
    // BP &= SI;
    BP = Alu16.And(BP, SI);
    CheckExternalEvents(cs8, 0xFBEA);
    // IN AL,DX (1000_FBE9 / 0x1FBE9)
    AL = Cpu.In8(DX);
    CheckExternalEvents(cs8, 0xFBEB);
    // IN AL,DX (1000_FBEA / 0x1FBEA)
    AL = Cpu.In8(DX);
    CheckExternalEvents(cs8, 0xFBEC);
    // IN AL,DX (1000_FBEB / 0x1FBEB)
    AL = Cpu.In8(DX);
    CheckExternalEvents(cs8, 0xFBED);
    // IN AL,DX (1000_FBEC / 0x1FBEC)
    AL = Cpu.In8(DX);
    CheckExternalEvents(cs8, 0xFBEF);
    // AAD 0xe5 (1000_FBED / 0x1FBED)
    new Instructions8(Cpu.State, Cpu, Memory, new(Memory, Cpu, Cpu.State)).Aad(0xE5);
    CheckExternalEvents(cs8, 0xFBF1);
    // OUT 0x20,AL (1000_FBEF / 0x1FBEF)
    Cpu.Out8(0x20, AL);
    CheckExternalEvents(cs8, 0xFBF3);
    // OUT 0xe5,AL (1000_FBF1 / 0x1FBF1)
    Cpu.Out8(0xE5, AL);
    CheckExternalEvents(cs8, 0xFBF5);
    // AND CH,AH (1000_FBF3 / 0x1FBF3)
    CH &= AH;
    CheckExternalEvents(cs8, 0xFBF7);
    // AAM 0x46 (1000_FBF5 / 0x1FBF5)
    new Instructions8(Cpu.State, Cpu, Memory, new(Memory, Cpu, Cpu.State)).Aam(0x46);
    CheckExternalEvents(cs8, 0xFBF8);
    // INC SI (1000_FBF7 / 0x1FBF7)
    SI++;
    CheckExternalEvents(cs8, 0xFBF9);
    // INC SI (1000_FBF8 / 0x1FBF8)
    SI++;
    CheckExternalEvents(cs8, 0xFBFA);
    // INC SI (1000_FBF9 / 0x1FBF9)
    SI++;
    CheckExternalEvents(cs8, 0xFBFB);
    // INC SI (1000_FBFA / 0x1FBFA)
    SI++;
    CheckExternalEvents(cs8, 0xFBFC);
    // INC SI (1000_FBFB / 0x1FBFB)
    SI++;
    CheckExternalEvents(cs8, 0xFBFD);
    // INC SI (1000_FBFC / 0x1FBFC)
    SI++;
    CheckExternalEvents(cs8, 0xFBFF);
    // SHL word ptr [BX + SI],CL (1000_FBFD / 0x1FBFD)
    UInt16[DS, (ushort)(BX + SI)] <<= CL;
    CheckExternalEvents(cs8, 0xFC01);
    ...
    CheckExternalEvents(cs8, 0xFC02);
